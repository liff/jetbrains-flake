...
def downloadUriFor(product: String, edition: Edition, status: Status, build: Build): Option[Uri] =
  (downloadPaths.get(product), packagePrefix.lift((product, edition))).mapN { (path, prefix) =>
    ...
    downloadSite / path / s"$prefix-$version.tar.gz"
  }
